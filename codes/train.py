...
def main():
    ...
    resume_state = None
    # todo ; is it really necessary to only do this if there's no resume state?
    if not opt['path']['resume_state']:
        util.mkdir_and_rename(opt['path']['experiments_root'])  # rename old folder if exists
        for key, path in opt['path'].items():
            # todo ; this code is quite smelly, just hardcode a list of opt keys you want to make dirs of
    ...
    # load the resume state dictionary to continue training
    if opt['path']['resume_state']:
        # resume training
        if os.path.isdir(opt['path']['resume_state']):
            resume_state_path = os.path.normpath(opt['path']['resume_state']) + '/*.state'
            resume_state_path = util.sorted_nicely(glob.glob(resume_state_path))[-1]
        else:
            resume_state_path = opt['path']['resume_state']
        resume_state = torch.load(resume_state_path)
        logger.info('Set [resume_state] to ' + resume_state_path)
        logger.info('Resuming training from epoch: {:,d}, iter: {:,d}'.format(
            resume_state['epoch'], resume_state['iter']
        ))
        option.check_resume(opt)  # check resume options

    # log the final training options
    logger.info('Training Options:\n' + option.dict2str(opt))

    # tensorboard logger
    if opt['use_tb_logger'] and 'debug' not in opt['name']:
        # get log dir argument name for tensorboardX >= 1.7 and < 1.6
        # todo ; ideally just the latest tensorboardX would be supported, with the version requirement
        #        being enforced by requirements.txt
        arg_name = [x for x in SummaryWriter.__init__.__code__.co_varnames if x in ["logdir", "log_dir"]][0]
        tb_logger = SummaryWriter(**{arg_name: '../tb_logger/' + opt['name']})

    # set seed
    seed = opt['train']['manual_seed']
    if not isinstance(seed, int):
        seed = random.randint(1, 10000)
    logger.info('{:s} seed: {:n}'.format("Manual" if isinstance(seed, int) else "Random", seed))
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            if train_loader is None:
                ...
                raise ValueError("Train dataloader is None for some reason.")
        elif phase == 'val':
    ...
    # load in resume state dict data
    start_epoch = 0
    current_step = 0
    virtual_step = 0
    if resume_state:
        ...
        if virtual_batch_size and virtual_batch_size > batch_size:
            virtual_step = virtual_step * virtual_batch_size / batch_size
            ...
            for n, train_data in enumerate(train_loader, start=1):
                ...
                if current_step % opt['logger']['save_checkpoint_freq'] == 0 and take_step:
                    ...
                    model.save_training_state(
                        epoch + (n >= len(train_loader)),
                        current_step,
                        opt['logger']['overwrite_chkp']
                    )
